...
//keeping integrity of force vector while enduring slider change
  ...
  //Calculate Torque
  torque = -1*sin(phi)*force1mag*pointApplied_x.value()/10;
  //when pointApplied_x.val = 100 , take to be 100mm, divide by 10 for N-cm torque calculation


  fill('black');
  text("Applied Force (F) = "+force1mag.toFixed(0)+" N-cm",100,20);
  text("Torque = "+torque.toFixed(0)+" N-cm",100,50);
